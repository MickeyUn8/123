...
# ========== CONFIGURATION ==========
TOKEN = '6863454476:AAGCem6I_wDrteHsfAoFqhp9yA_C_sFoRss'  # Your bot token
ADMIN_ID = 1069319252  # Only this user can approve/disapprove
...
# In-memory user storage: user_id -> { "plan": int, "valid_until": date, "approved_by": int, "time_approved": str, "username": str }
approved_users = {}
...
def create_inline_keyboard():
    ...
    button3 = types.InlineKeyboardButton(
        text="🪀 𝗝𝗢𝗜𝗡 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 🪀", url="https://t.me/+L35ubq-EsYdkMzk1")
    button1 = types.InlineKeyboardButton(text="💔 𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗢𝘄𝗻𝗲𝗿 💔",
        url="https://t.me/MickeyUnb")
    markup.add(button3)
    markup.add(button1)
    return markup
...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        if len(cmd_parts) < 4:
            bot.send_message(chat_id,
                             "⚠️ *Missing plan or days!*\n"
                             "ℹ️ *Format:* `/approve <user_id> <plan> <days>`",
                             reply_markup=create_inline_keyboard(), parse_mode='Markdown')
            return
        try:
            ...
            bot.send_message(chat_id,
                             "⚠️ *Plan and days must be integers!*",
                             reply_markup=create_inline_keyboard(), parse_mode='Markdown')
            return

        tz = pytz.timezone('Asia/Kolkata')
        now = datetime.now(tz)
        valid_until = (now + timedelta(days=days)).strftime("%Y-%m-%d")
        time_approved = now.strftime("%I:%M:%S %p %Y-%m-%d")
        ...
        bot.send_message(
            ...
            f"👤 *User ID:* `{target_user_id}`\n"
            f"🗑️ *The user's account has been disapproved.*",
            reply_markup=create_inline_keyboard(), parse_mode='Markdown')
        ...
        try:
            bot.send_message(
                target_user_id,
                "🚫 *Your account has been disapproved and removed.*\n"
    ...
    try:
        ...
        if not user_data:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Please contact the owner for assistance: @MickeyUnb.*", 
                                       reply_markup=create_inline_keyboard(), parse_mode='Markdown')
            return
        ...
        if valid_until_str:
            ...
            if now > valid_until:
                bot.send_message(chat_id, "*⚠️ Your access has expired!*\n"
...
def myinfo_command(message):
    ...
    if not user_data:
        response = ("*⚠️ No account information found. ⚠️*\n"
        ...
        button2 = types.InlineKeyboardButton(
            text="💸 𝗣𝗿𝗶𝗰𝗲 𝗟𝗶𝘀𝘁 💸", url="https://t.me/c/3766773009/2")
        markup.add(button1)
        markup.add(button2)
    else:
...
def rules_command(message):
    rules_text = ("*📜 Bot Rules - Keep It Cool!\n\n"
